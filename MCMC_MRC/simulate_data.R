#library(copula)
simulate_mixC_dim3 <- function(TT, nt) {
  U <- vector("list", length = TT)
  nts <- rep(nt, TT)
  for (t in 1:TT) {
    theta <- 2  # Clayton copula parameter (must be > 0)
    dim <- 3    # Number of dimensions
    clayton_cop <- claytonCopula(param = theta, dim = dim)
    # Generate samples
    n <- nts[t]
...
simulate_mixC_dim2 <- function(TT, nt) {
  U <- vector("list", length = TT)
  nts <- rep(nt, TT)

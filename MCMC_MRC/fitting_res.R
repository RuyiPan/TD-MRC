...
fig_pi <- df_Pi %>% ggplot( aes(x = Time)) +
  ...
  scale_x_continuous(breaks=seq(1,TT,by=5)) +
...
# dim(acc.all)
# plot(temp_res$acc.all[,4,8],type="l")
# plot(temp_res$Theta[range,11,1], type="l")
# 
# df_Theta$Mean
# /(2+df_Theta$Mean)
# plot(Theta_mean[[1]][1,]/(2+Theta_mean[[1]][1,]), type="l")

...
#for check 
job_num = 1
if (is.na(job_name)) job_name <- "running_time_sim"
...
runtime_table <- data.frame(
  ...
  percent_total_elapsed = 100 * as.numeric(time_parts) / time_taken_sim[["elapsed"]],
  row.names = NULL
)
runtime_table <- runtime_table[order(runtime_table$seconds, decreasing = TRUE), ]

print(time_taken_sim)
print(runtime_table)

filename_base <- paste0(job_name, "_job", job_num, "_runtime")
res <- list(
  job_name = job_name,
  job_num = job_num,
...
write.csv(runtime_table, file.path(path, paste0(filename_base, ".csv")), row.names = FALSE)

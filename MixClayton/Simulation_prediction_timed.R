...
time_counts <- setNames(rep(0, length(time_parts)), names(time_parts))

time_block <- function(name, expr) {
  start <- proc.time()[["elapsed"]]
  on.exit({
    time_parts[name] <<- time_parts[name] + proc.time()[["elapsed"]] - start
    time_counts[name] <<- time_counts[name] + 1
  })
  eval.parent(substitute(expr))
}

time_taken_mcmc <- system.time(for (b in 1:B) {
  print(b)
  time_block("adaptive", {
  ...
  for (it in 1:batch.size) {
    ...
    time_block("b_theta", {
      # (b) Posterior conditional for theta_tk (possible parallel)
      for (t in 1:TT) {
    ...
    time_block("c_pi", {
      # (c) Posterior conditional for pi_t
      for (t in 1:TT) {
    ...
    time_block("d_eta", {
      # (d) Posterior conditional for eta_t
      for (t in 1:TT) {
    ...
    time_block("e_omega", {
      # (e) Posterior conditional for omega
...
#WAIC
WAIC_value <- time_block("WAIC", WAIC(U_train, Theta[range,,], Pi[range,,]))
#LPML
LPML_value <- time_block("LPML", LPML(U_train, Theta[range,,], Pi[range,,]))
#DIC
DIC_value <- time_block("DIC", DIC(U_train, Theta[range,,], Pi[range,,]))



#get LPS
lps_result <- time_block("LPS", {
  ...
  for (k in range) {
    ...
    dir_w <- c0*p+colSums(rbind(Eta.all[k, eta_set, ]))+eta_t_plus
  ...
  list(predictive_dist=predictive_dist, LPS=LPS)
})

predictive_dist <- lps_result$predictive_dist
LPS_value <- lps_result$LPS

total_timed_seconds <- sum(time_parts)
runtime_table <- data.frame(
  part = names(time_parts),
  seconds = as.numeric(time_parts),
  minutes = as.numeric(time_parts) / 60,
  calls = as.integer(time_counts),
  seconds_per_call = as.numeric(time_parts) / pmax(as.integer(time_counts), 1),
  percent_total_timed = 100 * as.numeric(time_parts) / total_timed_seconds,
  row.names = NULL
)
runtime_table <- runtime_table[order(runtime_table$seconds, decreasing = TRUE), ]

print(time_taken_mcmc)
print(runtime_table)
...
res <- list(nts=nts, U_train=U_train, U_test=U_test, Theta=Theta, Pi=Pi, Eta.all=Eta.all, omega.all=omega.all,
            ...
            WAIC=WAIC_value, DIC=DIC_value, LPML=LPML_value,
            predictive_dist=predictive_dist, LPS=LPS_value,
            time_taken_mcmc=time_taken_mcmc, runtime_table=runtime_table)

filename<- paste0("job_name=", job_name,"job_num=", job_num, 
                  "LPS_mix_new_timed",".rds")

saveRDS(res, paste0(path,"/",filename))
write.csv(runtime_table,
          file.path(path, paste0("job_name=", job_name, "job_num=", job_num,
                                 "LPS_mix_new_timed_runtime.csv")),
          row.names = FALSE)

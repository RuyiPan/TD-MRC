...
Pi[1,,] <- matrix(rep(c(1,0,0,0),TT), TT, 4,byrow = T)

d <- c(1,1,1,1)*0.1 #parameter for theta
  ...
  for (it in 1:batch.size) {
    ...
    for (t in 1:TT) {
      ...
      prop_theta <- c(rgamma(1, ada.shape[t,1], ada.shape[t,1]/current_theta[1]),
                      ...
                      1,
                      1)
      prop_theta <- ifelse(prop_theta <= 10^(-4), 10^(-4), prop_theta)
...
for (k in range) {
  ...
  temp_dist <- apply(U_test[[1]], 1, function(row) l.mg(row,theta_t_plus)[1])
  predictive_dist <- cbind(predictive_dist, temp_dist)
}
predictive_dist_mean <- rowMeans(predictive_dist,na.rm = TRUE)
